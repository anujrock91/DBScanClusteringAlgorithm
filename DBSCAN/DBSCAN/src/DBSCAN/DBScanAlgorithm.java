...
public class DBScanAlgorithm {
	...
	public void getJaccardCoefficientAndRAND(String filePath, ArrayList<ArrayList<Integer>> clusters) throws IOException{
		...
		double RAND = ((double)M1100/(double)((PMatrix.length - countOfOutliers)*(PMatrix.length-countOfOutliers)));
		...
		for(int i=0;i<PMatrix.length;i++){
			for(int k=0;k<PMatrix[i].length;k++){
				if((PMatrix[i][k] == CMatrix[i][k]) && PMatrix[i][k] == 1 && (groundTruthClusters[i]!=-1 && groundTruthClusters[k]!=-1) ){
					++M11;
				}
			}
		}
